...
namespace Zephyr.Web.Areas.Pyn.Common
{
    public class PynHelper
    {
        ...
        public static dynamic GetEditUrls(string controller,object extend=null)
        {
            ...
            expando["getdetail"] =  string.Format("/api/pyn/{0}/getdetail/",controller);
            expando["getmaster"] =  string.Format("/api/pyn/{0}/geteditmaster/",controller);
            expando["edit"] = string.Format("/api/pyn/{0}/edit/", controller);
            expando["audit"] = string.Format("/api/pyn/{0}/audit/", controller);
            expando["getrowid"] = string.Format("/api/pyn/{0}/getnewrowid/", controller);
        ...
        {
            var expando = (IDictionary<string, object>)new ExpandoObject();
            expando["query"] = string.Format("/api/pyn/{0}", controller);
            expando["remove"] = string.Format("/api/pyn/{0}/", controller);
            expando["id"] = string.Format("/api/pyn/{0}/getnewid", controller);
            expando["audit"] = string.Format("/api/pyn/{0}/audit/", controller);
            expando["edit"] = string.Format("/pyn/{0}/edit/", controller);
            if (extend!=null)
                EachHelper.EachObjectProperty(extend, (i, name, value) => { expando[name] = value; });
